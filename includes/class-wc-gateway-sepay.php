...
class WC_Gateway_SePay extends WC_Payment_Gateway
{
    ...
    public function __construct()
    {
		...
		add_action('woocommerce_email_before_order_table', [$this, 'email_instructions'], 10, 3); // Err | Thêm
	...
	public function email_instructions($order, $sent_to_admin, $plain_text = false)
	{
		...
		// Lấy thông tin tài khoản và QR code như thankyou_page
		if ($this->api->is_connected() && $this->cached_bank_account_data) {
			$bank_account_id = $this->get_option('bank_account');
			$bank_account = $this->api->get_bank_account($bank_account_id);
			$account_number = $this->get_option('sub_account') ? $this->get_option('sub_account') : $bank_account['account_number'];
		...
		// Nội dung HTML cho email (có thể tuỳ chỉnh thêm)
        echo '<div style="margin-bottom:40px;text-align:center;">
                <h2 style="text-align:center;">Quét mã QR để thanh toán:</h2>
                <p style="text-align:center;font-weight:600;">
                    <img src="' . esc_url($qr_code_url) . '" alt="QR Code" style="max-width:250px;" />
                </p>';
        echo '  <h3 style="text-align:center;">Thông tin chuyển khoản ngân hàng:</h3>';
        echo '  <table class="info-table" style="width:100%;border:1px solid #e7e7e7;">
                    <tr>
                        <td style="width:50%;padding:5px;text-align:right;font-weight:600;">Tên tài khoản:</td>
                        <td style="width:50%;padding:5px;text-align:left;">' . esc_html($account_holder_name) . '</td>
                    </tr>
                    <tr>
                        <td style="padding:5px;text-align:right;font-weight:600;">Số tài khoản:</td>
                        <td style="padding:5px;text-align:left;">' . esc_html($account_number) . '</td>
                    </tr>
                    <tr>
                        <td style="padding:5px;text-align:right;font-weight:600;">Ngân hàng:</td>
                        <td style="padding:5px;text-align:left;">' . esc_html($displayed_bank_name) . '</td>
                    </tr>
                    <tr>
                        <td style="padding:5px;text-align:right;font-weight:600;">Số tiền:</td>
                        <td style="padding:5px;text-align:left;">' . wc_price($order->get_total()) . '</td>
                    </tr>
                    <tr>
                        <td style="padding:5px;text-align:right;font-weight:600;">Nội dung*:</td>
                        <td style="padding:5px;text-align:left;font-weight:600;">' . esc_html($remark) . '</td>
                    </tr>
                </table>';
        echo '</div>';
	}
