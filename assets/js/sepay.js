    ...
    jQuery.ajax({
        ...
        success: function (data) {
            ...
            if (data.status === true && (data.order_status === 'processing' || data.order_status === 'completed')) {
				...
				// Err | Thêm - Xóa thông báo cũ trước khi thêm mới
                jQuery('.sepay-message .paid-notification').remove();
                ...
                if (download_mode === 'auto') {
                    ...
                    // Err | Sửa
                    jQuery('.force-download').off('click').on('click', () => {
                        let urls = availableDownloads.map((download) => download.download_url);
                ...
                if (download_mode === 'manual') {
					...
					// Err | Thêm - Xóa bảng tải xuống cũ trước khi thêm mới
                    jQuery('.sepay-download .download-list').empty();
					
                    // Err | Sửa
                    function formatDate(date) {
                        // Nếu date là object có thuộc tính 'date', lấy giá trị đó
